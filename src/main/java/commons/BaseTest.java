...
public class BaseTest {
    ...
    /** Get browser follow browser factory pattern */
    ...
    /**
     * ThreadSafe - For all environments (local, cloud, mobile) with all browser
     * types
     */
    public WebDriver getBrowserDriverAllEnvironment(String env, String browserName, String browserVersion,
            String osName, String osVersion, String url) {
        switch (env) {
            case "local":
    ...
    /** 
     * For parallel execution with ITestContext - Thread-safe version
     * Uses context-based key strategy instead of ThreadLocal to support parallel=tests mode
     */
    public WebDriver getBrowserDriverWithContext(String env, String browserName, String browserVersion,
            String osName, String osVersion, String url, ITestContext context) {
        
        // Debug logging to verify parameters
        log.info("=== getBrowserDriverWithContext called ===");
        log.info("env={}, browserName={}, url={}", env, browserName, url);
        log.info("Test/Context: {}, Parallel: {}", context.getName(), context.getSuite().getParallel());
        
        WebDriver driver;
        switch (env) {
            case "local":
                log.info("Creating LOCAL driver with browser: {}", browserName);
        ...
        // Determine storage key based on parallel execution mode
        String suiteParallel = context.getSuite().getParallel().toString();
        String driverKey;

        if ("tests".equalsIgnoreCase(suiteParallel)) {
            // parallel=tests → key by context name (supports multiple tests on same thread)
            driverKey = context.getName();
            log.info("Created driver for TEST: {} on thread {}", context.getName(), Thread.currentThread().getId());
        } else if ("classes".equalsIgnoreCase(suiteParallel)) {
            // parallel=classes → key by thread (each class on own thread)
            driverKey = context.getName() + "-" + Thread.currentThread().getId();
            log.info("Created driver for CLASS: {} on thread {}", context.getName(), Thread.currentThread().getId());
        } else {
            // No parallel execution
            driverKey = context.getName();
            log.info("Created driver for SINGLE: {} on thread {}", context.getName(), Thread.currentThread().getId());
        }

        // Store in driverMap only - this is the single source of truth
        driverMap.put(driverKey, driver);
        
        // Also maintain ThreadLocal for backward compatibility with getDriver()
        tDriver.set(driver);
    ...
    public WebDriver getBrowserDriver3(String env, String browserName, String browserVersion, String osName,
            String osVersion, String url) {
        log.info(
                "getBrowserDriver3 called with env={}, browserName={}, browserVersion={}, osName={}, osVersion={}, url={}",
                env, browserName, browserVersion, osName, osVersion, url);
        switch (env == null ? "" : env.toLowerCase()) {
            ...
            // WebDriver mapped =
            // BrowserStackDriverMap.getCurrentThreadActiveRemoteWebdriver();
            // if (mapped == null) {
            // log.warn("BrowserStackDriverMap returned null for current thread. Falling
            // back to creating RemoteWebDriver via BrowserStackFactory.");
            // tDriver.set(new BrowserStackFactory(browserName, browserVersion, osName,
            // osVersion).createDriver());
            // } else {
            // tDriver.set(mapped);
            // }
            // } catch (Exception e) {
            // log.error("Exception while obtaining BrowserStack mapped driver; falling back
            // to BrowserStackFactory", e);
            // tDriver.set(new BrowserStackFactory(browserName, browserVersion, osName,
            // osVersion).createDriver());
            // }
            // break;
        ...
        if (driver == null) {
            ...
            throw new RuntimeException(
                    "Failed to initialize WebDriver: tDriver.get() returned null in getBrowserDriver3. Check BrowserStack SDK initialization or factory configuration.");
        }
        driver.manage().timeouts().implicitlyWait(Duration.ofSeconds(GlobalConstants.LONG_TIMEOUT));
        if (!browserName.toLowerCase().contains("mobile")) {
            configureDesktopWindowSafely(driver, browserName);
    ...
    public void closeBrowserDriver() {
        ...
        try {
            ...
            if (browserName != null) {
                ...
                log.warn("Could not determine browser driver process name from instance string: {}",
                        browserDriverInstanceName);
            }
    ...
    /** Environment Factory Pattern (threadlocal) */
    public void closeBrowserDriverThreadSafe() {
        WebDriver threadDriver = tDriver.get();
        if (threadDriver == null) {
            log.warn("closeBrowserDriverThreadSafe called but tDriver.get() is null");
            return;
        }
        ...
        try {
            ...
            if (browserName != null) {
                ...
                log.info("Thread {} driver process cleanup command exit code: {}", Thread.currentThread().getId(),
                        exitCode);
            } else {
    ...
    public void afterClass(ITestContext context) {
        closeBrowserDriverWithContext(context);
    }
    ...
    public static void removeDriver() {
        ...
        if (driver != null) {
            try {
                driver.manage().deleteAllCookies();
                driver.quit();
                System.out.println("Removed WebDriver instance " + driver.toString() + " for thread ID = "
    ...
    public void closeThreadDriver() {
        ...
        if (driver != null) {
            try {
                driver.manage().deleteAllCookies();
                System.out.println("AfterClass - Thread ID: " + Thread.currentThread().getId()
                        + " - Driver still active: " + driver.toString());
                driver.quit();
            } catch (Exception e) {
    ...
    private static void closeBrowserDriverWithContext(ITestContext context) {
    ...
    protected void assertEqualsWithMessage(Object actual, Object expected, String context) {
    ...
    protected void assertFalseWithMessage(boolean condition, String context) {
